...
def main():
    ...
    parser.add_argument(
        "--n-samples", type=int, default=500, help="Training samples (default: 500)"
    )
    parser.add_argument(
        "--n-features", type=int, default=8, help="Number of features (default: 8)"
    )
    parser.add_argument(
        "--n-trees", type=int, default=20, help="Number of estimators (default: 20)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=100,
        help="Samples to embed for prediction (default: 100)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default="both",
        help="Task type (default: both)",
    )
    parser.add_argument(
        "--models",
        nargs="+",
        default=["rf", "gbm", "lgbm", "catboost"],
        choices=["rf", "gbm", "lgbm", "catboost", "dt"],
        help="Models to run (default: rf gbm lgbm catboost)",
    )
    parser.add_argument(
        "--outdir", type=str, default="tmp", help="Output directory (default: tmp)"
    )
    ...
    def run(name, model, X_train, y_train, X_pl):
        ...
        n_trees = (
            ...
            or getattr(model, "iterations", None)
            or 1
        )
    ...
    if X_reg is not None:
        ...
        if "dt" in args.models:
            ...
            run(
                "dt_reg",
                DecisionTreeRegressor(max_depth=args.max_depth, random_state=args.seed),
                X_reg_df,
                y_reg,
                X_reg_pl,
            )
